...
real(8), dimension(3000000) :: Omega_Fastness,Gamma_Phi,Count_Rate,Flux,R_in,Small_a,H_0,C,Delta,BL_Thickness
real(8) :: Nustar,Gravitational_Constant,Mass_Solar,Mass_star,Speed_of_Light,Conversion_Factor
real(8) :: pi,Delta_Nu,Omega_rin,Parsec,Kiloparsec,Distance,A_1,A_2,mu,R_ISCO
real(8), dimension(4) :: Mass_Select,Radius_Select,Eps_Select
character*100 :: Sample_File(6),Source,Filename(10),SourceName(3000000)
...
Data Mass_Select / 1.4,1.4,1.4,1.4 /
Data Radius_Select / 1.0E+06,1.0E+06,1.0E+06,1.0E+06 /
...
do k=1,10
  ...
  do j=1,Line_Number
    read(11,*) Count_Rate(j),Lower_QPO_Frequency(j)
  ...
  do i=1,Number_of_Lines
    ...
    do j=1,Line_Number
      ...
      Upper_QPO_Frequency(j)=Lower_QPO_Frequency(j)+Delta_Nu 
      ...
      Omega_Fastness(j)=Omega_rin/Omega_Kepler_Alfven(j)
      ...
      BL_Thickness=Delta(j)*R_in(j)

      write(20+i,100) Mass(i),Radius(i),Magnetic_Field(i),Alpha_BL(i),Mdot(j)*1.0E-18,Epsilon(j),Gamma_Phi(j),&
      Delta(j),Delta_Nu*1.0E-03,Lower_QPO_Frequency(j),Delta_Nu/Lower_QPO_Frequency(j),BL_Thickness(j)*1.0E-06,R_in(j)/R_ISCO
    end do  
  end do
