...
real(8), dimension(5000) :: Mdot,Flux,Luminosity,Delta_Nu,Lower_QPO,Upper_QPO,rin,Omega_rin,Omega_Kepler_rin
real(8), dimension(5000) :: Small_Omega,Epsilon

open(unit=9,file='4U_1608_52_Flux_QPO_DeltaNu.dat')
open(10,file='Mdot_vs_Epsilon.dat')
Number_of_Lines=2560

do i=1,Number_of_Lines
    read(9,*) Flux(i),Delta_Nu(i),Lower_QPO(i),Upper_QPO(i)
end do
close(9)

Mass=1.35000E+00    
Radius=1.12000E+06        
Magnetic_Field=6.81808E+07            
Alpha_BL=3.90587E+00    
...
do i=1,Number_of_Lines
    ...
    A_1=2.0*pi*Upper_QPO(i)/Omega_Kepler_rin(i)
    A_2=((A_1**2.0)/(2.0*Small_Omega(i)))-(2.0*Small_Omega(i))+1.5
    Epsilon(i)=(1.0-Small_Omega(i))/((Alpha_BL)*(A_2))
    
    write(10,101) Mdot(i)*1.0E-18,Epsilon(i),Delta_Nu(i),Lower_QPO(i),Upper_QPO(i),Epsilon(i)*Alpha_BL
    101 format(6es15.5)
end do
