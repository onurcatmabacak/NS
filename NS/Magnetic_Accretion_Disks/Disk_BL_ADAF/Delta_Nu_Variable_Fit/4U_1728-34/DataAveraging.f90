...
integer :: i,j
real(8), dimension(5000) :: Mdot,Eps,Delta_Nu,Lower_QPO_Frequency,Upper_QPO_Frequency
real(8) :: Total_Mdot,Total_Epsilon,Total_Delta_Nu,Total_Lower_QPO_Frequency,Total_Upper_QPO_Frequency
real(8) :: Average_Mdot,Average_Epsilon,Average_Delta_Nu,Average_Lower_QPO_Frequency,Average_Upper_QPO_Frequency
...
do i=1,1399
    read(10,*) Mdot(i),Eps(i),Delta_Nu(i),Lower_QPO_Frequency(i),Upper_QPO_Frequency(i)
end do
close(10)
...
Frequency=minval(Lower_QPO_Frequency,(Lower_QPO_Frequency .gt. 0.0))
do while(Frequency .le. maxval(Lower_QPO_Frequency))
    Total_Mdot=0.0
    Total_Epsilon=0.0
    Total_Delta_Nu=0.0
    Total_Lower_QPO_Frequency=0.0
    Total_Upper_QPO_Frequency=0.0
    j=0
    do i=1,2560
        if(Lower_QPO_Frequency(i) .ge. Frequency .and. Lower_QPO_Frequency(i) .le. Frequency+Frequency_Range) then
            Total_Mdot=Total_Mdot+Mdot(i)
            Total_Epsilon=Total_Epsilon+(Eps(i)*Mdot(i))
            Total_Delta_Nu=Total_Delta_Nu+(Delta_Nu(i)*Mdot(i))
            Total_Lower_QPO_Frequency=Total_Lower_QPO_Frequency+(Lower_QPO_Frequency(i)*Mdot(i))
            Total_Upper_QPO_Frequency=Total_Upper_QPO_Frequency+(Upper_QPO_Frequency(i)*Mdot(i))
            j=j+1
        end if
    end do
    ...
    Average_Lower_QPO_Frequency=Total_Lower_QPO_Frequency/Total_Mdot
    Average_Upper_QPO_Frequency=Total_Upper_QPO_Frequency/Total_Mdot
    ...
    A_1=2.0*pi*Average_Upper_QPO_Frequency/Omega_Kepler_rin
    ...
    write(11,101) Average_Mdot*1.0E-18,Average_Epsilon,Epsilon,Average_Delta_Nu,Average_Lower_QPO_Frequency,&
    Average_Upper_QPO_Frequency,A,H_0
    101 format(8es15.5)
