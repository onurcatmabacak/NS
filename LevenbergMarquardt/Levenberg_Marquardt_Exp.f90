...
write(*,101) z(1),exp(z(2)),-exp(z(3)),ChiS
101 format(4e15.5)
...
do i=1,M
eps_fit_function(i)=(x(1)*(Mdot(i)**(2.0/7.0)))+(exp(x(2))*(Mdot(i)**(9.0/7.0)))-(exp(x(3))*(Mdot(i)**(10.0/7.0)))
...
write(*,101) onur,z(1),exp(z(2)),-exp(z(3))
101 format(1i3,3e15.5)
! ChiSquare_new calculation
do i=1,M
eps_fit_function_new(i)=((z(1))*(Mdot(i)**(2.0/7.0)))+(exp(z(2))*(Mdot(i)**(9.0/7.0)))&
-(exp(z(3))*(Mdot(i)**(10.0/7.0)))
...
do i=1,M
eps_fit_func(i)=z(1)*(Mdot(i)**(2.0/7.0))+exp(z(2))*(Mdot(i)**(9.0/7.0))-exp(z(3))*(Mdot(i)**(10.0/7.0))
