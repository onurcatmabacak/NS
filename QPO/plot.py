...
def data_process(n, case):
	...
	if case == 1:
		nu_a = np.array([random() for x in range(n)])*1450 + 250
	if case == 2:
		nu_a = np.array([random() for x in range(n)])*1050 + 150
...
for n in n_list:
	for case in [1,2]:
		...
		plt.savefig('qpo_n' + str(n) + '_case' + str(case) + '.pdf', bbox_inches='tight')
		plt.close()
		...
		plt.savefig('dist_n' + str(n) + '_case' + str(case) + '.pdf', bbox_inches='tight')
		plt.close()
